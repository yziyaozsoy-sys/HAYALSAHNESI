...
// ==========================================
// VİRAL MISRA SEÇİMİ & GÖRSEL SANAT KARTI MOTORU (GARANTİLİ TETİKLEYİCİ)
// ==========================================
...
function handleTextSelection() {
  ...
  // En az 4 harfli bir kelime veya mısra seçildiyse
  if (text.length >= 4) {
    selectedQuoteText = text;
    const range = selection.getRangeAt(0);
    const rect = range.getBoundingClientRect();

    // Seçimin hemen üstünde konumlandır
...
// Masaüstünde fareyi bırakınca, mobilde parmağı kaldırınca çalıştır
document.addEventListener('mouseup', () => setTimeout(handleTextSelection, 50));
document.addEventListener('touchend', () => setTimeout(handleTextSelection, 100));

// Boş bir yere basıldığında balonu kapat (Balonun butonlarına basılmadığı sürece)
